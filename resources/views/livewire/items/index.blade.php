    ...
    <div class="flex flex-wrap items-center divide-y md:divide-none">
        ...
        <div class="px-4 py-4 md:pb-4 m-auto w-72">
            <a href="{{ route('item.show', [$item->categories->first()->slug, $item->slug]) }}" class="">
                <img class='h-72 object-cover m-auto' src="{{ Storage::url($item->image->path) }}" alt="{{ $item->name }}" title="{{ $item->name }}">
            </a>
            <div class="flex justify-between space-x-2 mt-2">
                <div class="space-y-2">
                    <a href="{{ route('item.show', [$item->categories->first()->slug, $item->slug]) }}" class="font-bold">{{ $item->name }}</a>
                    <div class="">
